...
def rot45_to_square(rotated_diagonals, direction):
  ...
  for row in range(len(rotated_diagonals)):
    for element_index, value in enumerate(rotated_diagonals[row]):
      if row < n:
        i, j = row + element_index + 1 - len(rotated_diagonals[row]), n - 1 - row + element_index
      else:
        i, j = row - n + 1 + element_index, element_index
      square_matrix[i, j] = value
  return square_matrix if direction == -1 else np.rot90(square_matrix, 1)
